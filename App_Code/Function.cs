...
public class Function
{
    ...
    {
        var cmd = new MySqlCommand("SELECT * FROM tbluser WHERE flduserName=?USER AND fldpassword=?PASS");
        ...
        var dt = _dbConnection.GetData(cmd);
    ...
    {
        var check = new MySqlCommand($"select * from tblsignup where fldName='{username}'");
        var checkTable = _dbConnection.GetData(check);
    ...
    {
        var cmd = new MySqlCommand("SELECT * FROM tblsignup");
    ...
    {
        var cmd = new MySqlCommand("DELETE FROM tblsignup");
        _dbConnection.ModifyData(cmd);
    }
    ...
    {
        var cmd =
            new MySqlCommand("SELECT fldNumber, fldName, fldID from tblCountryStats ORDER BY fldNumber ASC limit 3");
        return _dbConnection.GetData(cmd);
    }
    ...
    {
        var cmd =
            new MySqlCommand(string.Format("UPDATE tblCountryStats set fldName='{1}', fldNumber={2} WHERE fldID={0}", id,
                name, number));
        _dbConnection.ModifyData(cmd);
    }
}